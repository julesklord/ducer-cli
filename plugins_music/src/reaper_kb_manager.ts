...
export class ReaperKBManager {
  private static readonly KB_DIR = path.join(
    os.homedir(),
    '.gemini-cli',
    'ducer',
    'reaper_kb',
  ...
  private static readonly KEYMAP_PATH = path.join(
    process.cwd(),
    'reactions_db',
    'db.ReaperKeyMap',
  );
  ...
  public async initialize(): Promise<void> {
    ...
    if (fs.readdirSync(ReaperKBManager.KB_DIR).length === 0) {
      logger.info('reaper_kb_init_start', {
        message: 'Initializing Reaper Action Database...',
      });
      await this.indexActions();
    }

    logger.info('reaper_kb_scripts_indexing', {
      message: 'Indexing Local Scripts Library...',
    });
  ...
  private async indexActions(): Promise<void> {
    if (!fs.existsSync(ReaperKBManager.KEYMAP_PATH)) {
      logger.error('reaper_kb_missing_keymap', {
        path: ReaperKBManager.KEYMAP_PATH,
      });
      return;
    }
    ...
    for (const line of lines) {
      const type = line.startsWith('ACT')
        ? 'ACT'
        : line.startsWith('SCR')
          ? 'SCR'
          : null;
  ...
  private categorizeAction(name: string): string {
    ...
    if (
      ...
      return 'items';
    if (n.includes('midi') || n.includes('note') || n.includes('quantize'))
      return 'midi';
    if (
      n.includes('fx') ||
      n.includes('mixer') ||
  ...
  private async indexScripts(): Promise<void> {
    ...
    const scripts = fs
      .readdirSync(ReaperKBManager.SCRIPTS_DIR)
  ...
  public searchActions(query: string, category?: string): ReaperAction[] {
    ...
    for (const cat of catsToSearch) {
      ...
      if (fs.existsSync(filePath)) {
        const actions: ReaperAction[] = JSON.parse(
