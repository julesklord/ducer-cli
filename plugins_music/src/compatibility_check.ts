...
export class CompatibilityShield {
   ...
   */
  public static validateGeminiClient(client: unknown): void {
    if (!client || typeof client !== 'object') {
      throw new Error(
        '[Ducer Maintenance Shield] CRITICAL: Invalid Gemini Client provided by host CLI.',
      );
    }
    ...
    if (missingMethods.length > 0) {
      throw new Error(
        `[Ducer Maintenance Shield] INCOMPATIBILITY DETECTED: 
...
Please contact the Ducer maintainer to update the plugin layer.`,
   ...
   */
  public static validateRuntime(): void {
    const versionParts = process.version.replace('v', '').split('.');
    const major = parseInt(versionParts[0], 10);

    if (major < 20) {
      throw new Error(
        `[Ducer Maintenance Shield] Node.js version ${process.version} is not supported. Ducer requires Node.js >= 20.`,
      );
    }
  }
}
