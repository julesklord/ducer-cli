...
export class ReaperBridgeClient {
    static REAPER_SCRIPTS_DIR = path.join(process.env['APPDATA'] || '', 'REAPER', 'Scripts');
    static CMD_FILE = path.join(ReaperBridgeClient.REAPER_SCRIPTS_DIR, 'ducer_commands.txt');
    static RESP_FILE = path.join(ReaperBridgeClient.REAPER_SCRIPTS_DIR, 'ducer_response.txt');
    ...
    async sendCommand(command) {
        ...
        if (!this.isBridgeAvailable()) {
            throw new Error('REAPER Bridge not found. Ensure Ducer bridge is installed in REAPER and check Web Control.');
        }
        // Write command
        ...
        try {
            ...
            for (const part of parts) {
                ...
                if (key === 'v')
                    status.version = val;
                if (key === 'state')
                    status.playState = parseInt(val, 10);
                if (key === 'cursor')
                    status.cursor = parseFloat(val);
                if (key === 'proj')
    ...
    async pollResponse(timeoutMs = 5000) {
...
//# sourceMappingURL=reaper_bridge_client.js.map