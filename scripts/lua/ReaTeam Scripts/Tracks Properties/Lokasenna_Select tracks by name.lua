--[[
    ...
    About:
        Allows quickly selecting multiple tracks based on a name search
        and several options.
...
local function get_settings()
    ...
    local selopts = GUI.Val("chk_selopts")
    local add = GUI.Val("chk_add")

    return {
        search = search,
        matchmultiple = matchopts[1],
        matchonlytop = matchopts[2],
        selchildren = selopts[1],
        selparents = selopts[2],
        selsiblings = selopts[3],
        add_selection = add
    }
...
local function continue_export(alias)
    ...
    local name = "Lokasenna_Select tracks by name - " .. alias
...
GUI.name = "Select tracks by name"
...
GUI.New("chk_add", "Checklist", {
    ...
    h = 32,
    caption = "",
    optarray = {"Add to existing selection"},
    dir = "v",
    pad = 4,
    font_a = 2,
    font_b = 3,
