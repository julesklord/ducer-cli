...
* ReaScript Name: BuyOne_Move pitch cursor to the first of selected notes.lua
...
* Notes: If start times of several notes are identical like in a chord for example, the first one is the highest, otherwise it's the one having the earliest start time
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	if notecnt > 0 then
		...
		if not sel then r.MB('No selected notes.','ERROR',0) end
	r.Undo_BeginBlock()
