...
* Notes: If start times of several notes are identical like in a chord for example, the last note is the lowest
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	if notecnt > 0 then
	local retval, sel, _, startpos, endpos, _, pitch, _ = r.MIDI_GetNote(take, notecnt-1)
