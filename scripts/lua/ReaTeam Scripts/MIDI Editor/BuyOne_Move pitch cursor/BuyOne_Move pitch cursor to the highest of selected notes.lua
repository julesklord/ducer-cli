...
* ReaScript Name: BuyOne_Move pitch cursor to the highest of selected notes.lua
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	local t = {}
		...
		local retval, sel, _, startpos, endpos, _, pitch, _ = r.MIDI_GetNote(take, i)
			if sel then t[#t+1] = pitch	end
		end
		if #t == 0 then r.MB('No selected notes.','ERROR',0) return end
	table.sort(t)
