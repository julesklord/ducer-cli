...
* ReaScript Name: BuyOne_Move pitch cursor to the first note.lua
...
* Notes: If start times of several notes are identical like in a chord for example, the first note is the highest
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	if notecnt > 0 then
	local retval, sel, _, startpos, endpos, _, pitch, _ = r.MIDI_GetNote(take, 0)
	r.MIDIEditor_SetSetting_int(hwnd, 'active_note_row', pitch)
	r.Undo_BeginBlock()
