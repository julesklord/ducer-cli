...
* ReaScript Name: BuyOne_Deselect only notes at pitch cursor.lua
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	local pitch_curs_pos = r.MIDIEditor_GetSetting_int(hwnd, 'active_note_row')
		...
		local retval, sel, _, startpos, endpos, _, pitch, _ = r.MIDI_GetNote(take, i)
			if pitch == pitch_curs_pos then r.MIDI_SetNote(take, i, false, muted, startpos, endpos, chan, pitch, vel,true) end
		end
	r.MIDI_Sort(take)
