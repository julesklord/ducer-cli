...
* ReaScript Name: BuyOne_Move edit and pitch cursors to the next note relative to edit cursor.lua
...
* Notes: If there's a note at or under the edit cursor, the edit cursor moves to the start of the closest note to the right along with the pitch cursor
...
local retval, notecnt, _, _ = r.MIDI_CountEvts(take)
	...
	reaper.PreventUIRefresh(1)
		...
		local start_time = r.MIDI_GetProjTimeFromPPQPos(take, startpos)
			if start_time > reaper.GetCursorPosition() then
