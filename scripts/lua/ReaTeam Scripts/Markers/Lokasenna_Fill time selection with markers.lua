--[[
Description: Fill time selection with markers
Version: 1.01
Author: Lokasenna
Donation: https://paypal.me/Lokasenna
Changelog:
    Fixed: Markers were all being numbered 0.
Links:
	Lokasenna's Website http://forum.cockos.com/member.php?u=10417
About: 
	Prompts the user for a number of markers, and a color, then fills the
	current time selection with that many markers, equally-spaced.
Extensions:
--]]
...
GUI.name = "Fill time selection with markers..."
GUI.x, GUI.y, GUI.w, GUI.h = 0, 0, 280, 80
GUI.anchor, GUI.corner = "mouse", "C"

local thread_URL = [[]]
local donate_URL = [[https://www.paypal.me/Lokasenna]]

GUI.fonts[3] = {"Calibri", 18, "b"}
GUI.fonts[4] = {"Calibri", 18, "b"}


GUI.colors["frm_col"] = {255, 0, 0, 255}
...
local function pick_color()
	...
	if retval ~= 0 then
		...
		GUI.colors["frm_col"] = {r / 255, g / 255, b / 255, 1}
		GUI.redraw_z[1] = true
...
local function add_markers()
	
	reaper.Undo_BeginBlock()
	
	local num_markers = tonumber(GUI.Val("txt_num"))
	
	if not num_markers or num_markers <= 0 then
		GUI.Val("txt_num", "")
		GUI.elms.txt_num.focus = true
		return 0
	end
	
	local start_time, end_time = reaper.GetSet_LoopTimeRange(false, false, 0, 0, false)
	
	local time_adj = (end_time - start_time) / (num_markers - 1)
	
	local color =  reaper.ColorToNative(GUI.colors.frm_col[1], GUI.colors.frm_col[2], GUI.colors.frm_col[3])

	-- reaper.AddProjectMarker2( proj, isrgn, pos, rgnend, name, wantidx, color )
	for i = 0, num_markers - 1 do

		local idx = reaper.AddProjectMarker2(0, false, start_time + (time_adj * i), 0, "new marker "..(i + 1), -1, color)		
		
	end
	
	reaper.Undo_EndBlock("Fill time selection with markers", -1)
...
GUI.elms = {
	...
	txt_num = GUI.Textbox:new(	1,	140, 8, 40, 20, "Number of markers:", 4),
	
	lbl_col = GUI.Label:new(	1,	48, 38, "Marker color:", 1, 3),
	frm_col = GUI.Frame:new(	1,	140, 36, 24, 24, true, true, "elm_frame", 0),
	
	btn_go = GUI.Button:new(	1,	204, 16, 48, 32, "Go!", add_markers),
...
function GUI.elms.frm_col:draw()
	
	GUI.Frame.draw(self)
	
	local x, y, w, h = self.x + 2, self.y + 2, self.w - 4, self.h - 4
	
	GUI.color("elm_bg")
	gfx.rect(x, y, w, h, true)	
	
	GUI.color("frm_col")
	gfx.rect(x + 2, y + 2, w - 4, h - 4, true)
	
end

function GUI.elms.frm_col:onmouseup()
