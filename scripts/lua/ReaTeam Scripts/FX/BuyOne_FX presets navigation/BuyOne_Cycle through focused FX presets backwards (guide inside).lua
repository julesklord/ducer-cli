...
* ReaScript name: BuyOne_Cycle through focused FX presets backwards (guide inside).lua
* Author: BuyOne
...
The script allows cycling through presets of the last focused FX in backward direction.
...
local ret, pres_cnt = table.unpack(t)
	...
	r.Undo_BeginBlock()
		...
		Switch_FX_Preset(tr, fx_num, cust_pres_list_t, INDEX_BASED_PRESET_SWITCH, false) -- forward false
		 _, fx_name = r.TrackFX_GetFXName(tr, fx_num, '') -- for undo caption
		_, pres_name = r.TrackFX_GetPreset(tr, fx_num, '') -- for undo caption
		elseif retval == 2 then
		Switch_FX_Preset(take, fx_num, cust_pres_list_t, INDEX_BASED_PRESET_SWITCH, false) -- forward false
