--[[
Description: Fill time selection with empty items
Version: 1.0
Author: Lokasenna
...
About: 
	Prompts the user for a number of items, and then fills the current
	time selection with that many empty MIDI items
Extensions:
--]]
...
GUI.name = "Fill time selection with empty items..."
GUI.x, GUI.y, GUI.w, GUI.h = 0, 0, 320, 56
...
local function add_items()
	...
	local num_items = tonumber(GUI.Val("txt_num"))
	...
	local time_adj = (end_time - start_time) / (num_items)

	local cur_track = reaper.GetSelectedTrack(0, 0) 

	for i = 0, num_items - 1 do
		
	local __ = reaper.CreateNewMIDIItemInProj(cur_track, start_time + (i * time_adj), start_time + ((i + 1) * time_adj))
		
	end
	
	reaper.Undo_EndBlock("Fill time selection with empty items", -1)
...
GUI.elms = {
	...
	btn_go = GUI.Button:new(	1,	224, 7, 48, 20, "Go!", add_items),
