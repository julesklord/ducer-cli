...
Mastertrk = reaper.GetMasterTrack( 0 )
fx_count =reaper.TrackFX_GetCount( Mastertrk )

for c = 0, fx_count-1 do
  local window = reaper.TrackFX_GetFloatingWindow( Mastertrk, c )
  if window ~= nil then 
    local FXstate = reaper.TrackFX_GetEnabled( Mastertrk, c )
    if FXstate then
      reaper.TrackFX_SetEnabled( Mastertrk, c, false )
    else 
      reaper.TrackFX_SetEnabled( Mastertrk, c, true )
    end
  end
end
