...
-- Mod by Eduserra
  ...
  ----User input default values-------------------------------------------------
    send_track_name = 'Rename'   ---- "Set the send track name"
    source_send = '1/2'       ---- "Set source send (x or x/y)"
    destination_send = '1/2'  ---- "Set destination send (x or x/y)"
    master_send = '1'         ---- "Master send on source tracks" --- Set 1 for enable or 0 for disable master send
    send_mode = '3'           ---- "Post-fd (0) pre-fx (1) post-fx (3)"
  ------------------------------------------------------------------------------
  ...
  ----Where will be the new send track, if no existed?--------------------------
    where_track = 4         ---- 1 - start, 2 - end of all tracks
  ...
  ----Other parameters for send track-------------------------------------------
    ...
    select_send_track = true
      show_fx_browser_for_selected_send_track = false
  ------------------------------------------------------------------------------
