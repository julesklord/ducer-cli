-- Name: Create post fader send from selected tracks to STEM 6 and disable master send
-- Date: 30-10-2023
-- Author: Bing Chat
...
function main()
    ...
    -- Find the track named "STEM 6"
    local stem_track = nil
    for i = 0, reaper.CountTracks(0) - 1 do
        local track = reaper.GetTrack(0, i)
        local _, track_name = reaper.GetSetMediaTrackInfo_String(track, "P_NAME", "", false)
        if track_name == "STEM 6" then
            stem_track = track
            break
        end
    end
    
    -- If "STEM 6" does not exist, show an error message and exit the script
    if not stem_track then
        reaper.ShowMessageBox("STEM 6 is missing", "Error", 0)
        return
    end
    ...
    for i = 0, count - 1 do
        ...
        -- Create a post-fader send from the current track to "STEM 6"
