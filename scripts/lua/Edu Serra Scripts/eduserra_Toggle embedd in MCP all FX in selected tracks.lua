-- @description Toggle selected tracks all FX embedded UI in MCP
-- @version 1.0
-- @author MPL
-- @mod by Edu Serra
...
scr_name_undo_hist = 'Toggle selected tracks all FX embedded UI in MCP' 
  ...
  for seltrackidx = 1, CountSelectedTracks(0) do
    ...
    for fx = 0, fx_count - 1 do
      local func = function(flag1,flag2) 
        ...
        local WAKflag2 = flag2 == 2 and 0 or 2 -- toggle the embedding state
...
function VF_CheckFunctions(vrs)
  ...
  if reaper.file_exists(SEfunc_path) then
    dofile(SEfunc_path)
    if not VF_version or VF_version < vrs then
      reaper.MB('Update '..SEfunc_path:gsub('%\\', '/')..' to version '..vrs..' or newer', '', 0)
    else
      return true
    end
  else
...
--------------------------------------------------------------------  
local ret = VF_CheckFunctions(3.26)
if ret then
  local ret2 = VF_CheckReaperVrs(6.25,true)
