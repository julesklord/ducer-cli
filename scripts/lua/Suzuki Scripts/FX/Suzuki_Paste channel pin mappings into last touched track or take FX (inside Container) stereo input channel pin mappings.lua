-- @description Paste channel pin mappings into last touched track or take FX (inside Container) stereo input channel pin mappings
-- @author Suzuki
-- @license GPL v3
-- @version 1.0
...
if itemidx ~= -1 then -- take FX
    ...
    r.TakeFX_SetPinMappings(take, fxidx, 0, 0, low32l, high32l) -- #3 input 0, #4 pin 0 left
    r.TakeFX_SetPinMappings(take, fxidx, 0, 1, low32r, high32r) -- #4 pin 1 right
    r.TakeFX_SetPinMappings(take, fxidx, 0, 0 + 0x1000000, n_low32l, n_high32l) -- #3 input 0, #4 pin 0 left
    r.TakeFX_SetPinMappings(take, fxidx, 0, 1 + 0x1000000, n_low32r, n_high32r) -- #4 pin 1 right
else --  track FX
    ...
    r.TrackFX_SetPinMappings(track, fxidx, 0, 0, low32l, high32l) -- #3 input 0, #4 pin 0 left
    r.TrackFX_SetPinMappings(track, fxidx, 0, 1, low32r, high32r) -- #4 pin 1 right
    r.TrackFX_SetPinMappings(track, fxidx, 0, 0 + 0x1000000, n_low32l, n_high32l) -- #3 input 0, #4 pin 0 left
    r.TrackFX_SetPinMappings(track, fxidx, 0, 1 + 0x1000000, n_low32r, n_high32r) -- #4 pin 1 right
end
r.Undo_EndBlock("Paste channel pin mappings into last touched FX's stereo input channel mappings", -1)