-- @description Set last touched track or take FX (inside Container) output stereo channels to 125-126
-- @author Suzuki
-- @license GPL v3
-- @version 1.0
...
local left = (15 - 1) * 2 -- each block has 16 stereo channel, which one?
local right = left + 1
...
if itemidx ~= -1 then -- take FX
  ...
  if isincontainer then
    ...
    if 126 > tonumber(hm_cch) then
        r.TakeFX_SetNamedConfigParm(take, parent_container, 'container_nch', 126)
    end
  else
    local hm_ch = r.GetMediaItemTakeInfo_Value(take, 'I_TAKEFX_NCH')
    if 126 > tonumber(hm_ch) then
        r.SetMediaItemTakeInfo_Value(take, 'I_TAKEFX_NCH', 126)
    end
  end
  ...
  if isincontainer then
    ...
    if 126 > tonumber(hm_cch) then
          r.TrackFX_SetNamedConfigParm(track, parent_container, 'container_nch', 126)
    end
  else
    local hm_ch = r.GetMediaTrackInfo_Value(track, 'I_NCHAN')
    if 126 > tonumber(hm_ch) then
          r.SetMediaTrackInfo_Value(track, 'I_NCHAN', 126)
    end
  end
...
r.Undo_EndBlock("Set last touched FX output stereo channels to 125-126", -1)