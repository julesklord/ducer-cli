...
local function TakeFX_SetPin(a, b, c, d, e, f, g, h)
  r.TakeFX_SetPinMappings(take, fxidx, 0, 0, low32l + a, high32l + e) -- #3 input 0, #4 pin 0 left
  r.TakeFX_SetPinMappings(take, fxidx, 0, 1, low32r + b, high32r + f) -- #4 pin 1 right
  r.TakeFX_SetPinMappings(take, fxidx, 0, 0 + 0x1000000, n_low32l + c, n_high32l + g) -- #3 input 0, #4 pin 0 left and add 0x1000000 to #4 for the second half blocks
  r.TakeFX_SetPinMappings(take, fxidx, 0, 1 + 0x1000000, n_low32r + d, n_high32r + h) -- #4 pin 1 right
end

local function TrackFX_SetPin(a, b, c, d, e, f, g, h)
  r.TrackFX_SetPinMappings(track, fxidx, 0, 0, low32l + a, high32l + e) -- #3 input 0, #4 pin 0 left
  r.TrackFX_SetPinMappings(track, fxidx, 0, 1, low32r + b, high32r + f) -- #4 pin 1 right
  r.TrackFX_SetPinMappings(track, fxidx, 0, 0 + 0x1000000, n_low32l + c, n_high32l + g) -- #3 input 0, #4 pin 0 left and add 0x1000000 to #4 for the second half blocks
  r.TrackFX_SetPinMappings(track, fxidx, 0, 1 + 0x1000000, n_low32r + d, n_high32r + h) -- #4 pin 1 right
end
...
if itemidx ~= -1 then -- take FX
  ...
  low32l, high32l = r.TakeFX_GetPinMappings(take, fxidx, 0, 0) -- #3 0 for input, #4 0 for left and for surround increase the value accordingly
  low32r, high32r = r.TakeFX_GetPinMappings(take, fxidx, 0, 1) -- #3 0 for input, #4 1 for right and for surround increase the value accordingly
  n_low32l, n_high32l = r.TakeFX_GetPinMappings(take, fxidx, 0, 0 + 0x1000000) -- #3 0 for input, #4 0 for left + 0x1000000 for another 64bits
  n_low32r, n_high32r = r.TakeFX_GetPinMappings(take, fxidx, 0, 1 + 0x1000000) -- #3 0 for input, #4 1 for right
  ...
  low32l, high32l = r.TrackFX_GetPinMappings(track, fxidx, 0, 0) -- #3 0 for input, #4 0 for left
  low32r, high32r = r.TrackFX_GetPinMappings(track, fxidx, 0, 1) -- #3 0 for input, #4 1 for right
  n_low32l, n_high32l = r.TrackFX_GetPinMappings(track, fxidx, 0, 0 + 0x1000000) -- #3 0 for input, #4 0 for left + 0x1000000 for another 64bits
  n_low32r, n_high32r = r.TrackFX_GetPinMappings(track, fxidx, 0, 1 + 0x1000000) -- #3 0 for input, #4 1 for right
  if block_num % 2 == 1 then -- odd number blocks -> #5 = low32bits
    if block_num == 1 then -- add 0x1000000 to #4 for the second half blocks
...
r.Undo_EndBlock("Add stereo channels to last touched FX input channel mappings", -1)
