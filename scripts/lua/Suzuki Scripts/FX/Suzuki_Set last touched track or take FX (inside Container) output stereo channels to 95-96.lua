-- @description Set last touched track or take FX (inside Container) output stereo channels to 95-96
-- @author Suzuki
-- @license GPL v3
-- @version 1.0
...
if itemidx ~= -1 then -- take FX
  ...
  if isincontainer then
    ...
    if 96 > tonumber(hm_cch) then
        r.TakeFX_SetNamedConfigParm(take, parent_container, 'container_nch', 96)
    end
  else
    local hm_ch = r.GetMediaItemTakeInfo_Value(take, 'I_TAKEFX_NCH')
    if 96 > tonumber(hm_ch) then
        r.SetMediaItemTakeInfo_Value(take, 'I_TAKEFX_NCH', 96)
    end
  end
    r.TakeFX_SetPinMappings(take, fxidx, 1, 0, 0, 0) -- remove the current mappings
    r.TakeFX_SetPinMappings(take, fxidx, 1, 1, 0, 0)
    r.TakeFX_SetPinMappings(take, fxidx, 1, 0 + 0x1000000, 2^left, 0) -- #3 output 1, #4 pin 0 left (add 0x1000000 for 3) and 4)), #6 set hi32bits for even number blocks, 2) and 4)
    r.TakeFX_SetPinMappings(take, fxidx, 1, 1 + 0x1000000, 2^right, 0) -- #4 pin 1 right
else --  track FX
  ...
  if isincontainer then
    ...
    if 96 > tonumber(hm_cch) then
          r.TrackFX_SetNamedConfigParm(track, parent_container, 'container_nch', 96)
    end
  else
    local hm_ch = r.GetMediaTrackInfo_Value(track, 'I_NCHAN')
    if 96 > tonumber(hm_ch) then
          r.SetMediaTrackInfo_Value(track, 'I_NCHAN', 96)
    end
  end
    r.TrackFX_SetPinMappings(track, fxidx, 1, 0, 0, 0) -- remove the current mappings
    r.TrackFX_SetPinMappings(track, fxidx, 1, 1, 0, 0)
    r.TrackFX_SetPinMappings(track, fxidx, 1, 0 + 0x1000000, 2^left, 0) -- #3 output 1, #4 pin 0 left (add 0x1000000 for 3) and 4)), #6 set hi32bits for even number blocks, 2) and 4)
    r.TrackFX_SetPinMappings(track, fxidx, 1, 1 + 0x1000000, 2^right, 0) -- #4 pin 1 right
end
r.Undo_EndBlock("Set last touched FX output stereo channels to 95-96", -1)