...
```mermaid
graph LR
    A[Intro: Percusión/Piano] --> B[Verso 1: Narrativa]
    B --> C[Coro: Gancho Melódico]
    C --> D[Verso 2: Desarrollo]
    D --> E[Solo de Trompeta/Piano]
    E --> F[Montuno: Pregón y Respuesta]
    F --> G[Cierre: Outro Narrativo]
```

### 🥧 Distribución de la Mezcla (Panorámica)

```mermaid
pie title Distribución Estereofónica
    "Voz Principal (C)": 30
    "Bajo / Bombo (C)": 20
    "Piano (L-R Wide)": 20
    "Percusión Menor (L-R)": 15
    "Metales / Coros (Side)": 15
```
...
2.  **EQ Quirúrgica:** Un pequeño "dip" de -1dB en los 450Hz para limpiar un
    poco de "mud" (turbidez) y dar más espacio a la voz líder.
3.  **Saturación de Cinta:** El uso de un emulador de cinta (Tape Saturation) a
    ...
    los metales.
4.  **Limitación:** Usar un limitador transparente (como el FabFilter Pro-L2)
    con un techo de -1.0 dBTP para asegurar la integridad de la señal en
